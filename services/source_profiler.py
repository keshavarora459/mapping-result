...
class SourceProfiler:
    ...
    DATE_PATTERNS = re.compile(r"(date|time|timestamp|dob|created|updated|year|month|day)", re.IGNORECASE)
    KEY_PATTERNS = re.compile(r"(id|key|code|num|number|guid|uuid)$", re.IGNORECASE)
    NUMERIC_PATTERNS = re.compile(r"(amount|price|cost|sales|revenue|qty|quantity|volume|rate|fee|tax|total|count|pnl)", re.IGNORECASE)
