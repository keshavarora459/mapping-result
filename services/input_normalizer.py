...
def normalize_dimension(raw: Any) -> Dict[str, Any]:
    ...
    if not name and not field_defs:
        return {}

    # A calculated dimension's definition is an expression, not a field name.
    expression = str(field_defs[0]) if field_defs else ""
    ...
    data_type = str(raw.get("dataType") or "STRING").upper()
    ...
    return {
        ...
        "qlik_expression": expression[1:] if expression.startswith("=") else expression,
