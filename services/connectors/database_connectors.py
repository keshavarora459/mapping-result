...
class SqlServerConnector(SourceConnector):
    ...
    def build_fabric_source(self, table_name: str, custom_sql: Optional[str] = None, columns: Optional[List[Dict[str, Any]]] = None) -> str:
        server = self.connection_details.get("server") or "server.database.windows.net"
        db = self.connection_details.get("database") or "Database"
...
class PostgresConnector(SourceConnector):
    ...
    def build_fabric_source(self, table_name: str, custom_sql: Optional[str] = None, columns: Optional[List[Dict[str, Any]]] = None) -> str:
        server = self.connection_details.get("server") or self.connection_details.get("host") or "postgres.database.azure.com"
        db = self.connection_details.get("database") or "Database"
...
class MySqlConnector(SourceConnector):
    ...
    def build_fabric_source(self, table_name: str, custom_sql: Optional[str] = None, columns: Optional[List[Dict[str, Any]]] = None) -> str:
        server = self.connection_details.get("server") or "mysql.database.azure.com"
        db = self.connection_details.get("database") or "Database"
        if custom_sql:
            sql_esc = escape_m_string(custom_sql)
